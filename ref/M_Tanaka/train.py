...
def mask_to_bb(Y):
    ...
    cx = ((left_col + right_col) /2.0)/gTargetImgSize[0]
    cy = ((top_row + bottom_row) /2.0)/gTargetImgSize[0]
    width = (abs(right_col - left_col))/gTargetImgSize[0]
    height = (abs(bottom_row - top_row))/gTargetImgSize[0]
...
def train_net(net,
              ...
              img_scale=0.5):
    # 1. Create dataset
    dataset = BasicDataset(dir_img, dir_mask, img_scale, force_samesize=True, target_size=gTargetImgSize)        #强制转换为512*512
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                true_bb = mask_to_bb(true_masks[0])
                true_bb = torch.from_numpy(true_bb).to(device=device, dtype=torch.float32)  # ndarry转tensor再放cuda上
                pred_bb = net(imgs)

                loss = nn.functional.smooth_l1_loss(pred_bb, true_bb)
                print("true_bb:", true_bb, "\t pred_bb:",pred_bb)
